...
def regression_bert():
  ...
  train_data, val_data, train_label, val_label = train_test_split(description_list, scaled_description_list, test_size=0.1)
  ...
  test_encodings = tokenizer(test_description_list, padding=True, truncation=True)

  test_dataset = DescriptionData(test_encodings, test_description_label_list)

  model = BertForSequenceClassification.from_pretrained("task1/output_finetuned_on_description_column", num_labels=1)
  ...
  price_series = price_series.values.reshape(-1, 1)
  scaled_price = min_max_scaler.fit_transform(price_series)
  param_dict = min_max_scaler.get_params()

  price_output = min_max_scaler.inverse_transform(test_output.predictions)

  pd.DataFrame({"Id": id_list, "Predicted": price_output.flatten().tolist()}).to_csv("task1/submission.csv", index=False)

  output_df = pd.concat([test_df, pd.Series(np.squeeze(test_output.predictions), name="scaled_description").to_frame()], axis=1)
